function [models,logP,RejectionPercentage]=gwmcmc(minit,logPfuns,mccount,varargin)
...
%  logPfuns: a cell of function handles returning the log probability of a
...
for wix=1:Nwalkers % wix = walker index
    
    for fix=1:NPfun % fix = function index
...
for row=1:Nkeep
    for jj=1:p.ThinChain
        ...
        zz=((p.StepSize - 1)*rand(1,Nwalkers) + 1).^2/p.StepSize; %draw a random number in the interval [1/lambda,lambda] with 1/sqrt(z) scaling
        ...
        if p.Parallel
            ...
            parfor wix=1:Nwalkers
                cp=curlogP(:,wix); % theta_t
                lr=logrand(:,wix); % ln(r)
                acceptfullstep=true; % default to accept step unless conditions below
                proposedlogP=nan(NPfun,1);
                
                % First check if ln(r) < (d-1)*ln(z). If not then
                % definitely reject. If true then still a chance we'll
                % accept the step.
                if lr(1) < ( numel(proposedm(:,wix)) - 1 )*log(zz(wix)) % if ln(r) < (d-1)*ln(z)
                    for fix=1:NPfun
                        proposedlogP(fix) = logPfuns{fix}(proposedm(:,wix)); %have tested workerobjwrapper but that is slower. % Get log(L(theta'))
...
RejectionPercentage = 100*sum(reject)/totcount;
display(RejectionPercentage)
